...
- **Frontend:** `index.html` (the app shell + connect gate, no build step — plain HTML/CSS/JS + ethers v5.7.2 from cdnjs, Reown AppKit via esm.sh) plus `keel-core.js` (shared pure/stateless core: contract addresses, network constants, `TOKEN_ABI`/`LEDGER_ABI`, federal-holiday date math). `keel-core.js` is loaded from `index.html`'s head via a **static `<script>` tag** (NOT `document.write` — Chrome's slow-network intervention silently drops document.write-injected scripts on mobile, which undefines the whole app and dead-clicks the UI). It must finish before the shell's inline script, which uses core globals (e.g. `MAX_CAP`) at top level. (Older docs/scripts may still say `keel-app.html` — the live file is `index.html`.)
...
- **GitHub Pages caches hard and rebuilds slowly (~2 min).** Always hard-refresh (Ctrl+Shift+R) and/or append `?v=N`. Stale deploys repeatedly masked whether fixes worked. A visible build marker was used during debugging and removed. Normal mobile Chrome serving a stale `index.html` while incognito works is the tell-tale symptom.
- **Auto-update guard (deploy step!):** a head script in `index.html` compares `APP_VERSION` against a never-cached `version.txt`; if the deploy is newer it reloads once with a `?v=` cache-buster (query guard prevents a loop). **On every deploy bump three values in lockstep: `APP_VERSION` in `index.html`, the value in `version.txt`, AND the `?v=` on the `keel-core.js` `<script>` tag** — forget the first two and stale clients won't auto-update; forget the core `?v=` and clients get a stale `keel-core.js`.
