...
- **Auto-update guard (deploy step!):** a head script in `index.html` compares `APP_VERSION` against a never-cached `version.txt`; if the deploy is newer it reloads once with a `?v=` cache-buster (query guard prevents a loop). **On every deploy bump `APP_VERSION` in `index.html` AND the value in `version.txt` together** — forget to bump and stale clients won't auto-update.
